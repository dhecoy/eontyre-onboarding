...
export default function OnboardingChat() {
  ...
  const handleSend = async () => {
    ...
    if (input.toLowerCase().includes('leverantör')) {
      ...
      aiResponse = 'Perfekt. Kontaktuppgifterna är sparade.';
      ...
      aiResponse = 'Tack! Vill du gå vidare till nästa steg?';
    }
    ...
    <div className="flex flex-col md:flex-row min-h-screen">
      <div className="w-full md:w-2/3 p-4 bg-gray-50 border-r">
        ...
        <div className="h-[400px] overflow-y-auto bg-white p-4 rounded shadow mb-4">
          {messages.map((msg, i) => (
            <div key={i} className={`mb-2 ${msg.sender === 'ai' ? 'text-left' : 'text-right'}`}>
              <span className={`inline-block px-3 py-2 rounded-lg ${msg.sender === 'ai' ? 'bg-blue-100 text-black' : 'bg-green-100 text-black'}`}>{msg.text}</span>
            </div>
          ))}
        </div>
        <div className="flex gap-2">
          ...
          />
          <button onClick={handleSend} className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Skicka</button>
        </div>
      </div>
